...
from pypgatk.proteogenomics.blast_get_position import BlastGetPositonService

log = logging.getLogger(__name__)

@click.command('blast_get_positon', short_help='Blast peptide and refence protein database to find variation sites.')
...
@click.pass_context
def blast_get_positon(ctx, config_file, input_psm_to_blast, output_psm, input_reference_database, canonical_peptide_prefix):
    ...
    if canonical_peptide_prefix is not None:
        pipeline_arguments[BlastGetPositonService.CONFIG_CANONICAL_PEPTIDE_PREFIX] = canonical_peptide_prefix

    blast_get_positon_service = BlastGetPositonService(config_data, pipeline_arguments)
    blast_get_positon_service.blast(input_psm_to_blast, output_psm, input_reference_database)