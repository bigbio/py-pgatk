...
from pypgatk.proteomics.db.decoy_fullprotein import ProteinDBService
from pypgatk.toolbox.exceptions import AppConfigException

this_dir, this_filename = os.path.split(__file__)


@click.command('generate-decoy-full', short_help='Create decoy protein sequences. Each protein '
...
@click.option('-t', '--temp_file',
              help='Set temporary file to write decoys prior to shuffling. Default=protein-decoy.fa')
...
def generate_database(ctx, config_file: str, output_database: str, input_database: str, cleavage_sites: str,
                      ...
                      max_iterations: int, do_not_shuffle: bool, keep_target_hits: bool, 
                      do_not_switch: bool, decoy_prefix: str, temp_file: str,
                      no_isobaric: bool):
  ...
  if keep_target_hits is not None:
    pipeline_arguments[ProteinDBService.CONFIG_KEEP_TARGET_HITS] = keep_target_hits
  ...
  proteindb_decoy_full = ProteinDBService(config_file, pipeline_arguments)
  proteindb_decoy_full.decoy_database_full_protein()