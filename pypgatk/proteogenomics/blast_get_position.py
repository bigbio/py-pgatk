...
class BlastGetPositionService(ParameterConfiguration):
    ...
    def __init__(self, config_data, pipeline_arguments):
        ...
        self._canonical_peptide_prefix = self.get_blast_parameters(variable=self.CONFIG_CANONICAL_PEPTIDE_PREFIX,
                                                                   default_value='sp,NP,ENSP')
        ...
        for fasta in fasta_set:
            if len(fasta) >= length:
                alignments_score = pairwise2.align.localms(sequenceA=fasta, sequenceB=peptide, match=1, mismatch=0,
                                                           open=-1, extend=0, score_only=True)
    ...
    def blast(self, input_psm_to_blast, output_psm):
        ...
        psms = pd.read_table(input_psm_to_blast, header=0, sep="\t")
        psms.loc[:, "position"] = psms.apply(lambda x: self._is_canonical(x["accession"]), axis=1)

        first_filter = psms[psms.position == "canonical"]
        psm_to_blast = psms[psms.position == "waiting for blast"]
        ...
        if len(psm_to_findpos) > 0:
            ...
            psm_to_findpos["position"] = psm_to_findpos.apply(
                lambda x: str(x["position"])[1:-1].split(",")[x["var_num"]], axis=1)
