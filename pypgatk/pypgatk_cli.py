...
@click.option('-t', '--tissue_type', help='Limit mutations to tissue types considered for generating proteinDBs, by default mutations from all tissue types are considered')
@click.option('-s', '--split_by_tissue_type', help='Use this flag to generate a proteinDB per tissue type as specified in the Primary site column, default is False')
@click.pass_context
def cosmic_to_proteindb(ctx, config_file, input_mutation, input_genes, output_db, tissue_type, split_by_tissue_type):
    ...
    pipeline_arguments = {CancerGenomesService.CONFIG_CANCER_GENOMES_MUTATION_FILE: input_mutation,
                          ...
                          CancerGenomesService.CONFIG_COSMIC_TISSUE_TYPE: tissue_type,
                          CancerGenomesService.CONFIG_COSMIC_SPLIT_BY_TISSUE: split_by_tissue_type}
