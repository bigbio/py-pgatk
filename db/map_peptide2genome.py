...
if len(sys.argv[1:])<=1:  ### Indicates that there are insufficient number of command-line arguments
    ...
    print("Example: python map_peptide2genome.py --input peptideTable.txt --gtf Homo_sapiens.GRCh37.90.gtf --fasta Homo_sapiens.GRCh37.90.pep.all.fa  --IDmap Ensembl90_IDmap.txt --output output_filename")
else:
    options, remainder = getopt.getopt(sys.argv[1:],'', ['input=','gtf=','fasta=','IDmap=','output=','pep_col=','prot_col='])
    for opt, arg in options:
        ...
        elif opt == '--pep_col': pep_col=int(arg)
        elif opt == '--prot_col': prot_col=int(arg)
        else:
