...
def threeframe_trasnlation(ctx, config_file, input, translation_table, output):
    ...
    pipeline_arguments = {EnsemblDataService.CONFIG_TRANSLATION_TABLE: translation_table, EnsemblDataService.PROTEIN_DB_OUTPUT: output}
...
@click.option('--config_file',
              ...
              help='Configuration to perform conversion between ENSEMBL Files',
              default='config/ensembl_config.yaml')
@click.option('-gf','--genome_fasta', help='Path to the genome sequence', required=True)
@click.option('-va','--vep_annotated_vcf', help='Path to the vep annotated VCF file', required=True)
@click.option('-gtf','--gene_annotated_gtf', help='Path to the gene annotations file', required=True)
...
@click.option('-rr', '--report_ref_seq', action='store_const', const=True, help='In addition to var peps, also report all ref peps')
@click.option('-out','--output_proteindb', default="peptide-database.fa", help="Output file name, exits if already exists")
@click.option('-afn','--annotation_field_name', default="CSQ", help="Annotation field name found in the INFO column, e.g CSQ or vep")
...
@click.option('-ti','--transcript_index', default=3, type=int,help='index of transcript ID in the annotated columns (separated by |)')
@click.option('-ci', '--consequence_index', default=1, type=int, help='index of consequence in the annotated columns (separated by |)')
@click.option('-eb','--excluded_biotypes', nargs='*', default=[], help="excluded_biotypes")
@click.option('-ec','--excluded_consequences', nargs='*', default=['downstream_gene_variant',
                                                                   'upstream_gene_variant', 'intergenic_variant',
                                                                   'intron_variant', 'synonymous_variant'],help="excluded_consequences")
@click.option('-scds','--skip_including_all_cds', action='store_const', const=True,help="by default any transcript that has a defined CDS will be used, this option disables this features instead it only depends on the biotypes")
@click.option('-ib','--included_biotypes', nargs='*', default=[], help="included_biotypes")
@click.option('-ic','--included_consequences', nargs='+', default=['all'], help="included_consequences, default all")
@click.option('-bs','--biotype_str', default='transcript_biotype', type=str,  help='string used to identify gene/transcript biotype in the gtf file.')
@click.pass_context
def vcf_to_proteindb(ctx, config_file, genome_fasta, vep_annotated_vcf, gene_annotated_gtf, nuclear_translation_table,
                     mito_translation_table, var_prefix, report_ref_seq, output_proteindb, annotation_field_name, af_field,
                     af_threshold, transcript_index, consequence_index, excluded_biotypes, excluded_consequences,
                     skip_including_all_CDSs, included_biotypes, included_consequences, biotype_str):

    if genome_fasta is None or vep_annotated_vcf is None or gene_annotated_gtf is None:
        print_help()

    pipeline_arguments = {}
    pipeline_arguments[EnsemblDataService.NUCLEAR_TANSLATION_TABLE] = nuclear_translation_table
